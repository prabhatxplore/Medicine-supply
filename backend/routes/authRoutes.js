...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype === "image/jpeg" || file.mimetype === "image/jpg") {
      ...
      cb(new Error("Only JPG images are allowed for nationalIdCard and citizenshipCard"));
    }
  },
});
...
router.post(
  ...
  [
    ...
    body("address").optional().trim().isLength({ max: 200 }).withMessage("Address max length is 200 chars"),
    body("phoneNumber").optional().trim().isLength({ min: 10, max: 15 }).withMessage("Phone number must be 10-15 digits"),
  ],
